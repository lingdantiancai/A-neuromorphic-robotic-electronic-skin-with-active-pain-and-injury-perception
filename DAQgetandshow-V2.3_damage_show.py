...
T_initial = time.time()
Time_duration_array=np.ones((3,8,8))
np.set_printoptions(precision=1)
Pulse_time_cache_array= np.zeros((3,8,8))+T_initial
                        ...
                        if result:
                            ...
                            for pulse, pulse_time in zip(Pulse_data, Pulse_time):
                                ...
                                if pw > 0.5:
                                    ...
                                    #色度0~255.
                                    # print(Dx,Dy,': ',freq/50*255)
                                    Pulse_time_cache_array[Da-1][Dx-1][Dy-1]= time.time()
                                    # print(Pulse_time_cache_array)
                                    #将压力分布阵列输出到队列queue
                                    queue.put(Pulse_time_cache_array)  



                                    # if [Dx,Dy] in collecting_pool:
                                    #     collecting_pool.remove([Dx,Dy])
                                    #     print(collecting_pool)
                                    #     print(len(collecting_pool))
...
def plot_data(queue):  
    ...
    pressure_matrix=np.random.rand(3, 8, 8)*255
    cache_pressure_matrix = pressure_matrix
    # 初始化图像显示  
    im1 = ax[0].imshow(pressure_matrix[0], cmap='gray', interpolation='nearest')  
    im2 = ax[1].imshow(pressure_matrix[1], cmap='gray', interpolation='nearest')   
    im3 = ax[2].imshow(pressure_matrix[2], cmap='gray', interpolation='nearest')   
    # im = ax.imshow(pressure_matrix, cmap='gray', interpolation='nearest')  
    fig2, ax2 = plt.subplots(1, 3, constrained_layout=True)

    # logo_ax = ax.inset_axes([0, 1.01, 0.3, 0.15])  # 这里的坐标和大小是相对于当前轴的  
    # logo_ax.imshow(log_img, extent=logo_extent, aspect='auto', zorder=10)  # zorder确保logo在上面  
    # logo_ax.set_axis_off()  # 关闭坐标轴 
    # plt.text(0.5, 1.05, 'Skin array', horizontalalignment='center', verticalalignment='bottom', transform=plt.gca().transAxes) 
    ...
    def update(frame):  
        ...
        #获取queue队列里的数据
        try:
            Pulse_time_cache_time = queue.get(block=False)
            Time_duration_array = time.time()  - Pulse_time_cache_time
            pressure_matrix = (1/Time_duration_array)*200
            # print(pressure_matrix)
            pressure_matrix = np.clip(pressure_matrix, 0, 255)  # 确保压力值在0-255之间
            im1.set_array(pressure_matrix[0])
            im2.set_array(pressure_matrix[1])
            im3.set_array(pressure_matrix[2])
        ...
        im1.set_data(colored_pressure_matrix[2])  # 更新图像数据 
        im2.set_data(colored_pressure_matrix[1])
        im3.set_data(colored_pressure_matrix[0])

            # 清除之前的文本
        ax2[0].clear()
        ax2[1].clear()
        ax2[2].clear()
        ax2[0].imshow(pressure_matrix[0], cmap='viridis', vmin=0, vmax=255) 
        ax2[1].imshow(pressure_matrix[1], cmap='viridis', vmin=0, vmax=255)
        ax2[2].imshow(pressure_matrix[2], cmap='viridis', vmin=0, vmax=255)
            # 在图像上添加文本
        for (i, j), val in np.ndenumerate(pressure_matrix[0]):
            ax2[0].text(j, i, f'{val:.1f}', ha='center', va='center', color='white')
        for (i, j), val in np.ndenumerate(pressure_matrix[1]):
            ax2[1].text(j, i, f'{val:.1f}', ha='center', va='center', color='white')
        for (i, j), val in np.ndenumerate(pressure_matrix[2]):
            ax2[2].text(j, i, f'{val:.1f}', ha='center', va='center', color='white')            
        # 刷新fig2
...
collecting_pool = [[m,n] for m in range(1,9) for n in range(1,9)]
