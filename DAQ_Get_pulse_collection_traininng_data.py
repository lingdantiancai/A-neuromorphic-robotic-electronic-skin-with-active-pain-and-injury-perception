...
if __name__ == "__main__":
    with DAQ122() as daq:
        ...
        if daq.config_adc_channel(DAQADCChannel.AIN1):
            ...
            pressure_matrix = np.random.rand(8, 8) * 255  # 生成0-255之间的随机数，模拟压力值  
            

            fig, ax = plt.subplots()  
              
            def update(frame):  
                global pressure_matrix   
                
                pressure_matrix += np.random.rand(8, 8) * 10 - 5   
                    ...
                    if n % 200 == 0:
                        ...
                        if result:
                            ...
                            for pulse, pulse_time in zip(Pulse_data, Pulse_time):
                                ...
                                y_smooth  = DL.smooth_data(y,20)
                                ps = DL.get_gradient(x,y_smooth)
                                ps_origin = DL.get_gradient(pulse_time,pulse)
                                pw = DL.get_pulse_width(x,y)
                                # ps = DL.get_gradient(x,y)
                                # print(f"Reading data {n}")

                                FQ = DL.get_peak_frequency(x,y)

                                # plt.plot(Pulse_time[0], Pulse_data[0])
                                # plt.plot(x, y)
                                # plt.plot(x, y_smooth)
                                # plt.ylim(-0.1,2.5)
                                # print(x[int(len(x)/2)])
                                # plt.text(x[int(len(x)/2)], 2.2, f'PS:{ps}', fontsize=12, color='red', ha='center')
                                # plt.text(x[int(len(x)/2)], 2.0, f'ps_origin:{ps_origin}', fontsize=12, color='red', ha='center')
                                # plt.show()
                                # break

                                # Ensure the training_data directory exists
                                directory = "./training_data_FQ"
                                if not os.path.exists(directory):
                                    os.makedirs(directory)
                                # with open("./training_data/AM_6.2k-PW_3k-PS_1nf.txt", "a") as file:
                                #     if pw > 0.5:
                                #         print(f"am:{am:.3f}, pw:{pw:.3f}, ps:{ps:.3f}, ps_origin:{ps_origin:.3f}",DL.locate_the_pulse(am,pw,ps))
                                #         file.write(f"am:{am:.3f}, pw:{pw:.3f}, ps:{ps:.3f} \n")
                                #         number_of_pulses += 1

                                with open("./training_data_FQ/FQ_2.0k-AM_3.9k-PW_2.4k-PS_1000nf.txt", "a") as file:
                                    if pw > 0.5:
                                        print(f"FQ:{FQ:.3f}, am:{am:.3f}, pw:{pw:.3f}, ps:{ps:.3f}, ps_origin:{ps_origin:.3f}",DL.locate_the_pulse(am,pw,ps))
                                        file.write(f"FQ:{FQ:.3f}, am:{am:.3f}, pw:{pw:.3f}, ps:{ps:.3f} \n")
                                        number_of_pulses += 1
                        ...
                        if number_of_pulses >= 200:
                            print("Accumulated 200 pulses, exiting program.")
                            number_of_pulses = 0
                            break
